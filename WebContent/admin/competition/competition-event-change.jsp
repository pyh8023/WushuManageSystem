<%@page import="com.pan.competition.bean.Event"%>
...
<%
	...
	Message<Event> message = eventService.getEventById(event_id);
	if(message.getCode().equals("200")){
		request.setAttribute("event", message.getData());
	}else{
...
%>
  <head>
    ...
    <link href="/WushuManageSystem/css/competition-event-add.css" rel="stylesheet">
  ...
  <body>
	...
	<ol class="breadcrumb">
	  ...
	  <li><a href="/WushuManageSystem/admin/competition/competition-msg.jsp?competition_id=${event.competition_id}">赛事项目信息</a></li>
	  <li><a href="#">赛事项目修改</a></li>
	</ol>
	
	<div class="container content  col-sm-10 col-sm-offset-1 col-md-8 col-md-offset-2">
		<h4 class="text-center"><b>修改赛事项目</b></h4>
		<div class="col-md-6 col-md-offset-3">
	    	<form class="form" action="/WushuManageSystem/servlet/EventServlet?action=update&event_id=${event.id}&competition_id=${event.competition_id}" method="post">
				  <div class="form-group">
				    <label for="event_name" class="control-label">项目名称</label>
				    <div>
				    	<input type="text" class="form-control" id="event_name" name="event_name" value="${event.name }" placeholder="请输入项目名称" required autofocus>
				    </div>
				  </div>
				  <div class="form-group">
				    <label for="event_number" class="control-label">项目编号</label>
				    <div>
				    	<input type="text" class="form-control" id="event_number"  value="${event.event_num }" name="event_number" placeholder="请输入项目编号" required>
				    </div>
				  </div>
				  <div class="form-group">
				  ...
				  <div class="form-group" id="event_max_athlet_num_div">
				    ...
				    <div>
				    	<input type="number" class="form-control" id="max_athlet_num" value="${event.max_athlet_num }" name="max_athlet_num" placeholder="请输入最大运动员人数" required/>
				    </div>
				  </div>
				  <div class="form-group" id="event_min_athlet_num_div">
				    <label for="min_athlet_num" class="control-label">最小运动员人数</label>
				    <div>
				    	<input type="number" class="form-control" id="min_athlet_num" name="min_athlet_num" value="${event.min_athlet_num }" placeholder="请输入最小运动员人数" required/>
				    </div>
				  </div>
				  <div class="form-group" id="event_max_male_athlet_num_div">
				    <label for="max_male_athlet_num" class="control-label">最大男运动员人数</label>
				    <div>
				    	<input type="number" class="form-control" id="max_male_athlet_num" name="max_male_athlet_num" value="${event.max_male_num }" placeholder="请输入最大男运动员人数" required/>
				    </div>
				  </div>
				  <div class="form-group" id="event_min_male_athlet_num_div">
				    <label for="min_male_athlet_num" class="control-label">最小男运动员人数</label>
				    <div>
				    	<input type="number" class="form-control" id="min_male_athlet_num" name="min_male_athlet_num" value="${event.min_male_num }" placeholder="请输入最小男运动员人数" required/>
				    </div>
				  </div>
				  <div class="form-group" id="event_max_female_athlet_num_div">
				    ...
				    <div>
				    	<input type="number" class="form-control" id="max_female_athlet_num" name="max_female_athlet_num" value="${event.max_female_num }" placeholder="请输入最大女运动员人数" required/>
				    </div>
				  </div>
				  <div class="form-group" id="event_min_female_athlet_num_div">
				    ...
				    <div>
				    	<input type="number" class="form-control" id="min_female_athlet_num" name="min_female_athlet_num" value="${event.min_female_num }" placeholder="请输入最小女运动员人数" required/>
				    </div>
				  </div>
				  <div>
				  	<button type="submit" class="btn btn-primary col-md-12" id="update_event_btn">修改</button>
				  </div>
				</form>
	    </div>
	</div>
    ...
    <script src="/WushuManageSystem/customjs/competition-event.js"></script>
    <script type="text/javascript">
	    var msg = "${ msg }";
		 ...
		 $("#event_group").val("${event.event_group}");
		 $("#event_sex").val("${event.sex}");
    </script>
 </body>
</html>